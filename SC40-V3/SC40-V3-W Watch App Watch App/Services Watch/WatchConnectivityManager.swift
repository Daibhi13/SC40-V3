...
extension WatchConnectivityManager: WCSessionDelegate {
    ...
    func session(_ session: WCSession, didReceiveApplicationContext applicationContext: [String : Any]) {
        ...
        guard !applicationContext.isEmpty else {
            print("⚠️ WatchConnectivityManager: Received empty application context")
            return
        }
    ...
    private func handleApplicationContext(_ context: [String: Any]) {
        ...
        guard !context.isEmpty else {
            print("⚠️ WatchConnectivityManager: Received empty context in handleApplicationContext")
            return
        }
        
        print("📱 WatchConnectivityManager: Received application context with keys: \(context.keys.joined(separator: ", "))")
        
        // Handle training data sync status
        if let syncStatus = context["trainingDataSynced"] as? Bool {
            trainingSessionsSynced = syncStatus
            if syncStatus {
                lastSyncDate = Date()
            }
            print("✅ WatchConnectivityManager: Updated training data sync status: \(syncStatus)")
        } else {
            print("⚠️ WatchConnectivityManager: No valid trainingDataSynced status found in context")
        }
    }
}
