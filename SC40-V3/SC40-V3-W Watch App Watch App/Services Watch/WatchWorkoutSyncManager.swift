...
class WatchWorkoutSyncManager: NSObject, ObservableObject {
    ...
    // INCREMENTAL BATCH HANDLING: Properties for assembling session batches
    private var incomingSessionBatches: [Int: [[String: Any]]] = [:]
    private var expectedTotalBatches: Int = 0
    private var expectedTotalSessions: Int = 0
...
extension WatchWorkoutSyncManager: WCSessionDelegate {
    ...
    private func handleIncomingMessage(_ message: [String: Any]) {
        print("📱 WatchWorkoutSyncManager: Processing message with keys: \(message.keys.joined(separator: ", "))")
        ...
        if let messageType = message["type"] as? String {
            ...
            case "training_sessions":
                handleTrainingSessions(message)
                return
            case "training_sessions_batch":
                handleTrainingSessionsBatch(message)
                return
            case "training_sessions_complete":
                handleTrainingSessionsComplete(message)
                return
            case "onboarding_complete":
                handleOnboardingComplete(message)
                return
            default:
                print("🔍 Processing standard message type: \(messageType)")
            }
        }
    ...
    func session(_ session: WCSession, didReceiveApplicationContext applicationContext: [String : Any]) {
        ...
        guard !applicationContext.isEmpty else {
            print("⚠️ WatchWorkoutSyncManager: Received empty application context")
            return
        }
        
        print("📱 WatchWorkoutSyncManager: Received application context with keys: \(applicationContext.keys.joined(separator: ", "))")
        
        // Handle session data
        if let sessionDataData = applicationContext["sessionData"] as? Data {
            do {
                let sessionData = try JSONDecoder.safeDecoder.decode(SessionDataSync.self, from: sessionDataData)
                DispatchQueue.main.async {
                    self.adaptSessionData(sessionData)
                }
                print("✅ WatchWorkoutSyncManager: Successfully processed session data from context")
            } catch {
                print("❌ WatchWorkoutSyncManager: Failed to decode session data from application context: \(error)")
            }
        } else {
            print("⚠️ WatchWorkoutSyncManager: No valid session data found in application context")
        }
        
        // Handle other potential context data
        if let proPickerData = applicationContext["proPickerData"] as? Data {
            do {
                let pickerData = try JSONDecoder.safeDecoder.decode(ProPickerDataSync.self, from: proPickerData)
                DispatchQueue.main.async {
                    self.adaptedProPickerData = pickerData
                }
                print("✅ WatchWorkoutSyncManager: Successfully processed pro picker data from context")
            } catch {
                print("❌ WatchWorkoutSyncManager: Failed to decode pro picker data: \(error)")
            }
        }
    }
}
...
extension WatchWorkoutSyncManager {
    ...
    private func handleTrainingSessions(_ message: [String: Any]) {
        print("🏃‍♂️ WatchWorkoutSyncManager: Processing training sessions")
        
        guard let sessionsData = message["sessions"] else {
            print("❌ No sessions data found in training_sessions message")
            return
        }
        
        // Handle both Data and Array formats
        if let data = sessionsData as? Data {
            // Data format - decode directly
            do {
                let sessions = try JSONDecoder.safeDecoder.decode([TrainingSession].self, from: data)
                DispatchQueue.main.async {
                    // Process decoded sessions
                    print("✅ Successfully decoded \(sessions.count) training sessions from Data")
                    self.processTrainingSessions(sessions)
                }
            } catch {
                print("❌ Failed to decode training sessions from Data: \(error)")
            }
        } else if let sessionsArray = sessionsData as? [[String: Any]] {
            // Array format - convert to TrainingSession objects
            do {
                let jsonData = try JSONSerialization.data(withJSONObject: sessionsArray)
                let sessions = try JSONDecoder.safeDecoder.decode([TrainingSession].self, from: jsonData)
                DispatchQueue.main.async {
                    print("✅ Successfully decoded \(sessions.count) training sessions from Array")
                    self.processTrainingSessions(sessions)
                }
            } catch {
                print("❌ Failed to decode training sessions from Array: \(error)")
                print("🔍 Sessions data type: \(type(of: sessionsData))")
            }
        } else {
            print("❌ Unknown sessions data format: \(type(of: sessionsData))")
        }
    }
    
    private func handleOnboardingComplete(_ message: [String: Any]) {
        print("🎯 WatchWorkoutSyncManager: Processing onboarding complete")
        
        // Extract onboarding data
        let name = message["name"] as? String ?? "Unknown"
        let level = message["level"] as? String ?? "Beginner"
        ...
        DispatchQueue.main.async {
            // Update any local state needed
            print("✅ WatchWorkoutSyncManager: Onboarding data processed successfully")
        }
    }
    
    private func processTrainingSessions(_ sessions: [TrainingSession]) {
        print("📋 Processing \(sessions.count) training sessions")
        
        // Log session details for debugging
        for (index, session) in sessions.enumerated() {
            print("📅 Session \(index + 1): Week \(session.week), Day \(session.day), Type: \(session.type), Focus: \(session.focus)")
            for sprint in session.sprints {
                print("   🏃‍♂️ Sprint: \(sprint.distanceYards)yd x\(sprint.reps) (\(sprint.intensity))")
            }
        }
        
        // Store sessions for use by workout views
        // You might want to save these to UserDefaults or a local store
        // For now, just confirm they were processed
        print("✅ All \(sessions.count) training sessions processed successfully")
    }
    
    // MARK: - Incremental Batch Handling
    
    private func handleTrainingSessionsBatch(_ message: [String: Any]) {
        print("📦 WatchWorkoutSyncManager: Processing training sessions batch")
        
        guard let batchIndex = message["batchIndex"] as? Int,
              let totalBatches = message["totalBatches"] as? Int,
              let sessionsData = message["sessions"] as? [[String: Any]] else {
            print("❌ Invalid batch message format")
            return
        }
        
        expectedTotalBatches = totalBatches
        incomingSessionBatches[batchIndex] = sessionsData
        
        print("✅ Received batch \(batchIndex + 1)/\(totalBatches) with \(sessionsData.count) sessions")
        print("📋 Progress: \(incomingSessionBatches.count)/\(totalBatches) batches received")
        
        // Check if we have all batches
        if incomingSessionBatches.count == totalBatches {
            print("🎉 All batches received! Assembling complete session list...")
            assembleCompleteSessions()
        }
    }
    
    private func handleTrainingSessionsComplete(_ message: [String: Any]) {
        print("✅ WatchWorkoutSyncManager: Training sessions transfer completed")
        ...
        if let totalSessions = message["totalSessions"] as? Int,
           let totalBatches = message["totalBatches"] as? Int {
            ...
            print("📊 Transfer summary: \(totalSessions) sessions in \(totalBatches) batches")
            
            // Final validation and assembly if not already done
            if incomingSessionBatches.count == totalBatches {
                assembleCompleteSessions()
            } else {
                print("⚠️ Missing batches: expected \(totalBatches), received \(incomingSessionBatches.count)")
            }
        }
    }
    
    private func assembleCompleteSessions() {
        print("🔧 Assembling \(expectedTotalBatches) batches into complete session list")
        
        var allSessionsData: [[String: Any]] = []
        
        // Assemble batches in correct order
        for batchIndex in 0..<expectedTotalBatches {
            if let batchData = incomingSessionBatches[batchIndex] {
                allSessionsData.append(contentsOf: batchData)
                print("✅ Added batch \(batchIndex) with \(batchData.count) sessions")
            } else {
                print("❌ Missing batch \(batchIndex)")
            }
        }
        
        print("📋 Total assembled sessions: \(allSessionsData.count)")
        
        // Convert to TrainingSession objects
        do {
            let jsonData = try JSONSerialization.data(withJSONObject: allSessionsData)
            let sessions = try JSONDecoder.safeDecoder.decode([TrainingSession].self, from: jsonData)
            
            DispatchQueue.main.async {
                self.processTrainingSessions(sessions)
                print("✅ Successfully processed \(sessions.count) sessions from incremental transfer")
            }
        } catch {
            print("❌ Failed to decode assembled sessions: \(error)")
        }
        
        // Clean up batch storage
        incomingSessionBatches.removeAll()
