...
class AppStartupManager: ObservableObject {
    ...
    private func updateUI(phase: StartupPhase? = nil, progress: Double? = nil, message: String? = nil, error: String? = nil, canProceed: Bool? = nil, isComplete: Bool? = nil) {
        DispatchQueue.main.async {
            if let phase = phase { self.startupPhase = phase }
            if let progress = progress { self.syncProgress = progress }
            if let message = message { self.syncMessage = message }
            if let error = error { self.syncError = error }
            if let canProceed = canProceed { self.canProceedToMainView = canProceed }
            if let isComplete = isComplete { self.isConnectivityCheckComplete = isComplete }
        }
    }
    ...
    private func showSplashScreen() {
        updateUI(phase: .splash, message: "Sprint Coach 40")
    ...
    private func initConnectivityCheck() {
        ...
        updateUI(phase: .connectivityCheck, progress: 0.1, message: "Checking device connectivity...")
    ...
    private func sendTrainingPlanToWatch() async throws {
        ...
        // Get actual user profile data from UserDefaults with direct handling
        let userLevel = self.getUserLevel()
        ...
        let trainingLevel = self.stringToTrainingLevel(userLevel)
    ...
    private func getUserLevel() -> String {
        ...
        if let level = UserDefaults.standard.string(forKey: "SC40_UserLevel"), !level.isEmpty {
            return level
        }
        if let level = UserDefaults.standard.string(forKey: "userLevel"), !level.isEmpty {
            return level
        }
        if let level = UserDefaults.standard.string(forKey: "user_level"), !level.isEmpty {
            return level
        }
        
        // Preserve user selection - don't override with default
        print("⚠️ AppStartupManager: No level found in UserDefaults, using Beginner as safe fallback")
        return "Beginner"
    }
    
    private func stringToTrainingLevel(_ levelString: String) -> TrainingLevel {
