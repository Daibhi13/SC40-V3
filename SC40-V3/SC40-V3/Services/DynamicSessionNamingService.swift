...
// MARK: - Dynamic Session Naming Service
...
extension DynamicSessionNamingService {
    ...
    func replaceLegacySessionType(_ legacyType: String, userLevel: String, distance: Int) -> String {
        ...
        case "Maximum Velocity":
            return getLevelSpecificFocus(level: userLevel.lowercased(), distance: distance, dayInWeek: 1)
            
        case "Progressive Distance":
            return getLevelSpecificFocus(level: userLevel.lowercased(), distance: distance, dayInWeek: 2)
...
extension DynamicSessionNamingService {
    ...
    /// Generate session naming based on UserProfile
    func generateForUserProfile(
        profile: UserProfile,
        distance: Int,
        reps: Int,
        intensity: String
        ...
        return generateSessionConfiguration(
            userLevel: profile.level,
            distance: distance,
            reps: reps,
            intensity: intensity,
            weekNumber: profile.currentWeek,
            dayInWeek: profile.currentDay
        )
    }
}
