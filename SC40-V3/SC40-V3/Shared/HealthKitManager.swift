    ...
    private func getUserProfile(completion: @escaping @Sendable (Int?, Double?, HKBiologicalSex?) -> Void) {
        ...
        if let weightType = HKQuantityType.quantityType(forIdentifier: .bodyMass) {
            ...
            let query = HKSampleQuery(
                ...
                // Store weight value for later use (avoid mutation in Sendable closure)
                let weightValue = latestSample.quantity.doubleValue(for: .gramUnit(with: .kilo))
                DispatchQueue.main.async {
                    profileData.weight = weightValue
                }
            }
