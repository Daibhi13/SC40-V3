...
struct PhoneWorkoutFlowView: View {
    ...
    enum WorkoutPhase: String, CaseIterable {
        ...
        var icon: String {
            ...
            case .drills: return "figure.flexibility"
...
struct PhaseProgressView: View {
    let phase: PhoneWorkoutFlowView.WorkoutPhase
    let isCurrent: Bool
    ...
    var body: some View {
        VStack(spacing: 6) {
            ZStack {
                Circle()
                    .fill(isCompleted ? phase.color : (isCurrent ? phase.color.opacity(0.3) : Color.gray.opacity(0.2)))
            ...
            Text(phase.rawValue)
                .font(.caption2)
                .foregroundColor(isCurrent ? phase.color : (isCompleted ? .green : .gray))
                .multilineTextAlignment(.center)
                .fixedSize(horizontal: false, vertical: true)
        }
        .frame(minWidth: 60)
