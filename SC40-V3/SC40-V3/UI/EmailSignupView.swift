...
struct EmailSignupView: View {
        ...
        ZStack {
                ...
                VStack(spacing: 32) {
                    ...
                    VStack(spacing: 16) {
                        ...
                        if authManager.isLoading {
                            Button(action: {
                                // CRASH PROTECTION: Safe haptic feedback
                                do {
    ...
    private func handleContinue() {
        ...
        print("📧 handleContinue called with: \(trimmedName), \(trimmedEmail)")
        ...
        // Additional safety check - prevent multiple rapid taps
        authManager.isLoading = true
        
        // CRASH PROTECTION: Safe haptic feedback with error handling
        ...
        // CRASH FIX: Dismiss sheet FIRST, then call success handler
        print("📧 Dismissing EmailSignupView sheet first to prevent race condition")
        dismiss()
        
        // Reset loading state immediately
        authManager.isLoading = false
        
        // Call success handler after dismissal with delay to prevent crash
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
            print("📧 Calling onSuccess callback with: \(trimmedName), \(trimmedEmail)")
            onSuccess(trimmedName, trimmedEmail)
        }
        
        // Optional: Perform background authentication after UI flow completes
        Task.detached { [weak authManager] in
            guard let authManager = authManager else {
                print("⚠️ AuthManager deallocated, skipping background authentication")
                return
            }
            
            do {
                await authManager.authenticate(with: .email)
