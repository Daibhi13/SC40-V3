...
/// DEPRECATED: This view is replaced by UnifiedAppFlowView
/// EntryIOSView created multiple entry points and complex navigation chains
/// UnifiedAppFlowView provides a single, clean entry point for the entire app
@available(*, deprecated, message: "Use UnifiedAppFlowView instead")
struct EntryIOSView: View {
    // CRASH PROTECTION: Initialize AppStartupManager safely
    @StateObject private var startupManager: AppStartupManager = {
        do {
            return AppStartupManager.shared
        } catch {
            print("⚠️ AppStartupManager failed to initialize: \(error)")
            return AppStartupManager.shared
        }
    }()
    ...
    var body: some View {
        if showContentView {
            ...
            WelcomeView(onContinue: { name, email in
                // CRASH PROTECTION: Store user data safely with error handling
                print("🔄 EntryIOSView: Storing user data - name: \(name), email: \(email ?? "nil")")
                
                // Use UserDefaultsManager for consistency
                UserDefaultsManager.shared.setValue(name, forKey: "welcomeUserName")
                if let email = email {
                    UserDefaultsManager.shared.setValue(email, forKey: "welcomeUserEmail")
                }
                
                // CRASH PROTECTION: Navigate immediately without delay to prevent race conditions
...
// TODO: Implement RecordCardView before enabling
/*
