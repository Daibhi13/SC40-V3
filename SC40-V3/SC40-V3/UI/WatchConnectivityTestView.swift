...
struct WatchConnectivityTestView: View {
    ...
    private func runPingTest() async {
        ...
        do {
            let success = await withCheckedContinuation { continuation in
                watchConnectivity.sendMessage(["test": "ping"]) { success in
            ...
            await MainActor.run {
                let result = TestResult(
                    ...
                    details: success ? "Successfully pinged Apple Watch" : "Failed to ping Apple Watch",
                    timestamp: Date()
                )
            ...
            await MainActor.run {
                let result = TestResult(
                    ...
                    success: false,
                    duration: Date().timeIntervalSince(testStartTime ?? Date()),
                    details: "Ping test failed: \(error.localizedDescription)",
                    timestamp: Date()
                )
                addTestResult(result)
                isRunningTests = false
    ...
    private func runForceSyncTest() async {
        ...
        do {
            ...
            await MainActor.run {
                let result = TestResult(
                    id: UUID(),
                    testName: "Force Sync Test",
                    success: true,
                    duration: Date().timeIntervalSince(testStartTime ?? Date()),
                    details: "Successfully forced sync to watch",
                    timestamp: Date()
                )
                addTestResult(result)
                isRunningTests = false
            ...
            await MainActor.run {
                let result = TestResult(
                    ...
                    success: false,
                    duration: Date().timeIntervalSince(testStartTime ?? Date()),
                    details: "Force sync failed: \(error.localizedDescription)",
                    timestamp: Date()
                )
                addTestResult(result)
                isRunningTests = false
    ...
    private func run7StageFlowTest() async {
        ...
        do {
            // Simulate a 7-stage workout flow
            for stage in 1...7 {
                try await Task.sleep(nanoseconds: 500_000_000) // 0.5s delay between stages
                
                let success = await withCheckedContinuation { continuation in
                    watchConnectivity.sendMessage(["stage": stage]) { success in
                        continuation.resume(returning: success)
                    }
                }
                
                if !success {
                    throw NSError(domain: "WatchConnectivityTest", code: stage, userInfo: [NSLocalizedDescriptionKey: "Failed at stage \(stage)"])
                }
            }
            ...
            await MainActor.run {
                let result = TestResult(
                    ...
                    success: true,
                    duration: Date().timeIntervalSince(testStartTime ?? Date()),
                    details: "Successfully completed all 7 stages",
                    timestamp: Date()
                )
                addTestResult(result)
                isRunningTests = false
            ...
            await MainActor.run {
                let result = TestResult(
                    ...
                    success: false,
                    duration: Date().timeIntervalSince(testStartTime ?? Date()),
                    details: "7-stage flow failed: \(error.localizedDescription)",
                    timestamp: Date()
                )
                addTestResult(result)
                isRunningTests = false
    ...
    private func runDataSyncTest() async {
        ...
        do {
            ...
            let success = await withCheckedContinuation { continuation in
                watchConnectivity.sendMessage(["syncData": testData]) { success in
                    continuation.resume(returning: success)
                }
            }
            ...
            await MainActor.run {
                let result = TestResult(
                    ...
                    success: success,
                    duration: Date().timeIntervalSince(testStartTime ?? Date()),
                    details: success ? "Data synced successfully" : "Data sync failed",
                    timestamp: Date()
                )
                addTestResult(result)
                isRunningTests = false
            ...
            await MainActor.run {
                let result = TestResult(
                    ...
                    success: false,
                    duration: Date().timeIntervalSince(testStartTime ?? Date()),
                    details: "Data sync failed: \(error.localizedDescription)",
                    timestamp: Date()
                )
                addTestResult(result)
                isRunningTests = false
