...
struct SCStarterProSession: Identifiable, Codable {
    var id: UUID = UUID()
    var numReps: Int
    ...
    init(numReps: Int, restInterval: TimeInterval, customWarmUp: [SCStarterProPhase]? = nil, customCoolDown: [SCStarterProPhase]? = nil) {
        ...
        self.phases = SCStarterProSession.defaultPhases(numReps: numReps, restInterval: restInterval, customWarmUp: customWarmUp, customCoolDown: customCoolDown)
    }
    ...
    static func defaultPhases(numReps: Int, restInterval: TimeInterval, customWarmUp: [SCStarterProPhase]? = nil, customCoolDown: [SCStarterProPhase]? = nil) -> [SCStarterProPhase] {
        ...
        phases.append(.strides(count: 3, distance: 20, intensity: 0.7)) // yards
        phases.append(.stretch(targets: ["Hamstrings", "Quads", "Hip Flexors", "Calves"]))
        // Drills
        phases.append(.chooseDrill)
        phases.append(.strides(count: 2, distance: 20, intensity: 0.9)) // yards
        // 40-Yard Time Trial Block
        for i in 1...numReps {
            phases.append(.starterCue(rep: i))
            phases.append(.sprint40yd(rep: i)) // 40 yards
...
// All distances are now in yards
