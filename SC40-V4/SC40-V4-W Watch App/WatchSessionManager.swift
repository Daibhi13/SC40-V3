...
class WatchSessionManager: NSObject, ObservableObject, WCSessionDelegate, @unchecked Sendable {
    ...
    private func startHeartbeat() {
        ...
        heartbeatTimer = Timer.scheduledTimer(withTimeInterval: 30.0, repeats: true) { @MainActor [weak self] _ in
            Task { @MainActor in
    ...
    func requestTrainingSessions() {
        ...
        if sessionRequestRetryCount < maxRetryAttempts {
            sessionRequestTimer = Timer.scheduledTimer(withTimeInterval: 30.0, repeats: false) { @MainActor [weak self] _ in
                Task { @MainActor in
                    guard let self = self else { return }
    ...
    private func scheduleRegularSyncChecks() {
        ...
        Timer.scheduledTimer(withTimeInterval: 120.0, repeats: true) { @MainActor [weak self] _ in
            Task { @MainActor in
                guard let self = self else { return }
    ...
    func forceSyncFromPhone() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 10.0) { @MainActor in
            if self.trainingSessions.isEmpty {
    ...
    private func processSyncTokenExchange(remoteToken: String, remoteDeviceId: String) {
        ...
        if remoteToken != self.syncToken {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { @MainActor in
