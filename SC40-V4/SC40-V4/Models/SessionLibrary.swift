...
extension WeeklyProgramTemplate {
    ...
    /// Generates a program for the user based on their level and frequency
    static func generateProgramForUser(level: String, frequency: Int) -> [WeeklyProgramTemplate] {
        // Filter sessions by level
        let levelSessions = sessionLibrary.filter { $0.level == level }
        
        // Generate 12 weeks of programs
        var programs: [WeeklyProgramTemplate] = []
        for week in 1...12 {
            let sessions = generateWeeklySessions(for: frequency, level: level, week: week)
            let program = WeeklyProgramTemplate(level: level, weekNumber: week, totalDays: frequency, sessions: sessions)
    ...
    private static func generateWeeklySessions(for frequency: Int, level: String, week: Int) -> [DaySessionTemplate] {
        let levelSessions = sessionLibrary.filter { $0.level == level }
        var sessions: [DaySessionTemplate] = []
        
        for day in 1...frequency {
            // Alternate session types based on day and week
            let sessionTypes: [LibrarySessionType] = [.sprint, .activeRecovery, .rest, .benchmark]
            let type = sessionTypes[(week + day) % sessionTypes.count]
            
            let template = levelSessions.filter { $0.sessionType == type }.randomElement()
