...
final class UserProfileViewModel: ObservableObject, @unchecked Sendable {
    ...
    // Public getter for allSessions
    var sessions: [UUID: TrainingSession] {
        allSessions
    }
    ...
    /// Refreshes the adaptive training program based on user's level and frequency
    func refreshAdaptiveProgram() {
        // Use SessionLibrary to generate program based on level and days
        let programs = WeeklyProgramTemplate.generateProgramForUser(level: profile.level, frequency: profile.frequency)
        let trainingSessions = convertWeeklyProgramsToTrainingSessions(programs)
        
        // Update allSessions with new program
        allSessions.removeAll()
        for session in trainingSessions {
            allSessions[session.id] = session
        }
        
        // Update profile with session IDs
        profile.sessionIDs = trainingSessions.map { $0.id }
        
        print("🔄 Refreshed adaptive program: \(trainingSessions.count) sessions for \(profile.level) level, \(profile.frequency) days/week")
    }
}
