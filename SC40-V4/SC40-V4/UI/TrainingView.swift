...
struct TrainingView: View {
        ...
        ZStack {
            ...
            NavigationView {
                ZStack {
                    ...
                    case .news:
                        AnyView(Text("News View - Coming Soon!")) // Placeholder for NewsView
                    case .shareWithTeammates:
    ...
    func mainDashboard(profile: UserProfile, userProfileVM: UserProfileViewModel) -> some View {
        ...
        if !userProfileVM.sessions.isEmpty {
            // User has a generated program
            sessionsToShow = Array(userProfileVM.sessions.values).prefix(84).sorted { (a, b) in
                if a.week == b.week { return a.day < b.day } else { return a.week < b.week }
            }
        } else {
            // Show static mock data for demo/onboarding incomplete - representative of SessionLibrary quality
            sessionsToShow = staticMockSessions
        }
