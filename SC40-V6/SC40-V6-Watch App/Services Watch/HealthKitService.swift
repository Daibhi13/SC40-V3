...
class WatchHealthKitService: NSObject, ObservableObject {
    ...
    override init() {
        // Initialize objectWillChange before calling super.init()
    ...
    private func startHeartRateMonitoring() {
        ...
        ) { @MainActor [self] _, samples, _, _, error in
            DispatchQueue.main.async {
                self.processHeartRateSamples(samples, error: error)
            }
        }
        
        heartRateQuery?.updateHandler = { @MainActor [self] _, samples, _, _, error in
            DispatchQueue.main.async {
                self.processHeartRateSamples(samples, error: error)
            }
        }
    ...
    @MainActor private func processHeartRateSamples(_ samples: [HKSample]?, error: Error?) {
        if let error = error {
