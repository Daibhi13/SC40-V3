...
// MARK: - WorkoutWatchViewModel for Watch
...
class WorkoutWatchViewModel: NSObject, ObservableObject, CLLocationManagerDelegate {

    // MARK: - WCSessionDelegate Implementation
    #if canImport(WatchConnectivity)
    @objc(session:activationDidCompleteWithState:error:) nonisolated func session(_ session: WCSession, activationDidCompleteWith activationState: WCSessionActivationState, error: Error?) {}
