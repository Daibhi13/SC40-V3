...
struct SyncableTrainingSession: Codable {
    let session: WatchTrainingSession
    var syncState: SyncState
    var lastSyncAttempt: Date?
    var syncRetryCount: Int
    
    init(from session: WatchTrainingSession) {
    ...
    func toWatchTrainingSession() -> WatchTrainingSession {
        return session
    }
}
...
@MainActor class WatchSessionManager: NSObject, ObservableObject, WCSessionDelegate, @unchecked Sendable {
    ...
    @Published var currentWorkoutSession: WatchTrainingSession? = nil
    ...
    private func createSessionsBasedOnUserData(pb: Double, level: String) -> [WatchTrainingSession] {
        // Always include Time Trial
        var sessions = [
            WatchTrainingSession(
                week: 0,
                day: 0,
                type: "Time Trial",
                focus: "Performance Test",
                sprints: [WatchSprintSet(distanceYards: 40, reps: 1, intensity: "max")],
                accessoryWork: []
            )
        ]
        ...
        case "beginner":
            sessions.append(contentsOf: [
                WatchTrainingSession(
                    ...
                    sprints: [WatchSprintSet(distanceYards: 20, reps: 3, intensity: "max")],
                    accessoryWork: ["Dynamic Warm-up", "Cool-down Stretching"]
                ),
                WatchTrainingSession(
                    week: 1, day: 2, type: "Speed Endurance", focus: "Basic Conditioning",
                    sprints: [WatchSprintSet(distanceYards: 30, reps: 2, intensity: "submax")],
        ...
        case "intermediate":
            sessions.append(contentsOf: [
                WatchTrainingSession(
                    week: 1, day: 1, type: "Speed Development", focus: "Acceleration",
                    sprints: [WatchSprintSet(distanceYards: 20, reps: 4, intensity: "max")],
                    accessoryWork: ["Plyometrics", "Dynamic Warm-up"]
                ),
                WatchTrainingSession(
                    week: 1, day: 2, type: "Speed Endurance", focus: "Lactate Tolerance",
                    sprints: [WatchSprintSet(distanceYards: 30, reps: 3, intensity: "max")],
        ...
        case "advanced":
            sessions.append(contentsOf: [
                WatchTrainingSession(
                    ...
                    sprints: [WatchSprintSet(distanceYards: 20, reps: 5, intensity: "max")],
                    accessoryWork: ["Advanced Plyometrics", "Speed Mechanics"]
                ),
                WatchTrainingSession(
                    week: 1, day: 2, type: "Speed Endurance", focus: "Power Endurance",
                    sprints: [
                        WatchSprintSet(distanceYards: 40, reps: 1, intensity: "max"),
                        WatchSprintSet(distanceYards: 30, reps: 2, intensity: "max"),
                        WatchSprintSet(distanceYards: 20, reps: 1, intensity: "max")
                    ],
    ...
    private func startHeartbeat() {
        ...
        heartbeatTimer = Timer.scheduledTimer(withTimeInterval: 30.0, repeats: true) { [self] _ in
            Task {
                self.sendHeartbeat()
            ...
            sessionRequestTimer = Timer.scheduledTimer(withTimeInterval: 30.0, repeats: false) { [weak self] _ in
                Task { @MainActor in
                    ...
                    if noSessions && noInstallError {
                        ...
                        self.requestWatchTrainingSessions()
                    }
                }
            }
        }
    ...
    func generateFallbackSessions() {
        ...
        var fallbackSessions: [WatchTrainingSession] = []
    ...
    private func generateLevelSpecificSession(week: Int, day: Int, level: String, frequency: Int) -> WatchTrainingSession {
        ...
        return WatchTrainingSession(
            ...
            sprints: [WatchSprintSet(distanceYards: distance, reps: finalReps, intensity: intensity)],
    ...
    func setCurrentWorkoutSession(_ session: WatchTrainingSession) {
    ...
    private func processingSessionsData(_ sessionsData: Data, from message: [String: Any]) {
        do {
            let sessions = try JSONDecoder().decode([WatchTrainingSession].self, from: sessionsData)
    ...
    func forceIPhoneSessionSync() {
        ...
        if sessionSource == "Fallback" {
            ...
            sessionRequestRetryCount = 0
            lastRetryTime = nil
            logger.info("🔄 Sending requestWatchTrainingSessions to iPhone...")
            requestWatchTrainingSessions()
    ...
    private func loadStoredSessionsWithSync() {
        ...
        if let data = UserDefaults.standard.data(forKey: "scheduledSessions"),
           let sessions = try? JSONDecoder().decode([WatchTrainingSession].self, from: data) {
            ...
            // Update UI sessions
            trainingSessions = syncableSessions.map { $0.toWatchTrainingSession() }
        ...
        for week in 1...2 {
            for day in 1...actualFrequency {
                let session = WatchTrainingSession(
                    ...
                    sprints: [WatchSprintSet(distanceYards: 20, reps: 2, intensity: "80%")],
    ...
    private func handleFullSessionData(_ reply: [String: Any]) {
        ...
        do {
            let receivedSessions = try JSONDecoder().decode([WatchTrainingSession].self, from: sessionsData)
    ...
    private func storeSessionsLocally(_ sessions: [WatchTrainingSession]) {
        do {
