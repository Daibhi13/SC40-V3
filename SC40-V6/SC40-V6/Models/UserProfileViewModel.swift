...
final class UserProfileViewModel: ObservableObject, @unchecked Sendable {
    ...
    init() {
        if let data = UserDefaults.standard.data(forKey: userDefaultsKey),
           let loaded = try? JSONDecoder().decode(UserProfile.self, from: data) {
            self.profile = loaded
            log("Loaded user profile for \(loaded.name)")
        } else {
            ...
            log("Created new user profile for \(newProfile.name)")
    ...
    private func saveProfile() {
        do {
            ...
            log("Profile saved successfully for \(self.profile.name)")
        } catch {
            log("Failed to save profile: \(error.localizedDescription)")
            Task { @MainActor in
    ...
    func refreshAdaptiveProgram() {
                print("Generating adaptive program for level: \(self.profile.level), frequency: \(self.profile.frequency) days/week")
        ...
        log("Generated \(trainingSessions.count) real training sessions across 12 weeks")
    ...
    private func sendSessionsToWatch() {
        ...
        print("Watch sync: \(upcomingSessions.count) sessions, data size: 0")
    ...
    func updatePersonalBest(_ time: Double) {
        Task { @MainActor in
            do {
                // Simple validation: time should be positive and reasonable
                if time <= 0 || time > 10 {
                    throw NSError(domain: "InvalidTime", code: 0, userInfo: [NSLocalizedDescriptionKey: "Time must be between 0 and 10 seconds"])
                }
                ...
                print("Logged personal best update for 40yd: old \(oldTime ?? 0), new \(time)")
                log("Updated personal best to \(String(format: "%.2f", time))s")
                
            } catch {
                print("Error updating personal best: \(error.localizedDescription)")
            }
        }
    }
    ...
    func verifyPersonalBestConsistency() {
        ...
        log("Personal Best Verification: PB=\(pb40yd?.description ?? "nil"), Baseline=\(String(format: "%.2f", baseline))")
        
        // Auto-fix if there's a mismatch
        if let pb = pb40yd, pb != baseline {
            log("Auto-fixing PB mismatch: setting baselineTime to match personalBests['40yd']")
            profile.baselineTime = pb
        } else if pb40yd == nil && baseline > 0 {
            log("Auto-fixing missing personalBests['40yd']: setting from baselineTime")
